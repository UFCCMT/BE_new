...
if [ $# -lt 2 ]; then
        ...
        echo "Usage: ./val-vul-gen.sh <jobscript_folder> <data_folder>"
        echo "x---x---x---x---x---x---x---x---x"
        exit 0
fi
...
#E_SIZE="15 20 25"
EPP="1,1,1" #"7,5,2;5,5,5;6,6,5"
CART="4,4,2;8,4,4;8,8,8;16,8,8;16,16,8;16,16,16"
PHY_PARAM=5
    ...
    do
      ...
      if [ $ELE_SIZE -le 13 ];
 	  ...
 	  #then
	    ...
	    echo '#MSUB -N job-bonebe-es'$ELE_SIZE'ec'$Exyz'np'$NUM_TASKS'.job' >>jobfile	#Job name
	    ...
	    echo '#MSUB -o '$2'/bonebe-es'$ELE_SIZE'ec'$Exyz'np'$NUM_TASKS'.csv' >> jobfile	#STDOUT
	    echo '#MSUB -e '$2'/bonebe-es'$ELE_SIZE'ec'$Exyz'np'$NUM_TASKS'.err' >> jobfile	#STDERR
	    echo " " >> jobfile
...
do
  ...
  do
    ...
    do
	  ...
	  #if [ $EL_X = $EL_Y ] && [ $EL_Y = $EL_Z ]; 
 	  #then
	    msub $1/job-bonebe'-es'$ELE_SIZE'ec'$Exyz'np'$NUM_TASKS'.job'
	  #fi
	  done
  done
done
