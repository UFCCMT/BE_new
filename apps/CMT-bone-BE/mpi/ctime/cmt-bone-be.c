...
int main (int argc, char *argv[])
{
  ...
  double t_steps[TIMESTEPS], t_avg, t_sum = 0;
#endif
...
*/
  ...
  for ( t = 0; t < TIMESTEPS; t++ ) {
    ...
    for (r = 0; r < RK; r++) {
      ...
      /* --------------------------- Compute (A) --------------------------- */
#ifdef PROFILE
      if (rank == PROBED_RANK) { tcompA_s = now(); }
#endif
      /* For each element owned by this rank: */
      for ( e = 0; e < ELEMENTS_PER_PROCESS; e++ ) {
...
#ifdef PROFILE
      if (rank == PROBED_RANK) { tcomm_s = now(); }
#endif
      ...
      /* --------------------------- Compute (B) --------------------------- */
#ifdef PROFILE
      if (rank == PROBED_RANK) { tcompB_s = now(); }
#endif
