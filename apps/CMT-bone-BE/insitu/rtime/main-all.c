...
int main (int argc, char *argv[])
{
  ...
  unsigned int T = (params->TIMESTEPS) * (params->RK) * (params->ELEMENTS_PER_PROCESS) * (params-> PHYSICAL_PARAMS);
  ...
  double t_comminit[ (params->TIMESTEPS) * (params->RK) ];
  double t_comminitaxis[ (params->TIMESTEPS) * (params->RK) * (CARTESIAN_DIMENSIONS)];
  double t_prepface[  (params->TIMESTEPS) * (params->RK) ];
  double t_cleanface[ (params->TIMESTEPS) * (params->RK) ];
  ...
  for ( t = 0; t < params->TIMESTEPS; t++ ) {  /* For each timestep: */
    for (r = 0; r < params->RK; r++) { /* For each of the three 'stages': */
      ...
      Ecomminit = now();
      t_comminit[h] = tdiff(Scomminit, Ecomminit);
      h++;
      ...
      t_prepface[x] = tmp_prepface/ntransfer;
      t_cleanface[x] = tmp_cleanface/ntransfer;
      x++;
  ...
  if ( rank == params->PROBED_RANK) {
    ...
    for (c=0; c < (params->RK)*(params->TIMESTEPS) ; c++){
  ...
  if ( rank == params->PROBED_RANK) {
    for (c=0; c < (params->RK)*(params->TIMESTEPS)*CARTESIAN_DIMENSIONS ; c++){
       tmp_comminitaxis += t_comminitaxis[c];
       if ( (c+1)%( (params->RK) * (CARTESIAN_DIMENSIONS)) == 0 && c!=0) {
