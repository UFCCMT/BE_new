...
do
    ...
    do
	  ...
	  echo "#PBS -l nodes=1" >> jobfile
	  echo '#PBS -N job-bonebe-es'$ELE_SIZE'ec'$Exyz'.job' >> jobfile	#Job name
	  echo "#PBS -l walltime=$wtime" >> jobfile
	  echo "#PBS -A CSC188" >> jobfile
	  echo '#PBS -o '$2'/bonebe-es'$ELE_SIZE'ec'$Exyz'.csv' >> jobfile	#STDOUT
	  echo '#PBS -e '$2'/bonebe-es'$ELE_SIZE'ec'$Exyz'.err' >> jobfile	#STDERR
	  echo "" >> jobfile 
	  echo " cd $MEMBERWORK" >> jobfile
	  echo "" >> jobfile
	  echo "aprun -n1 /ccs/home/arav1994/compute-only/cmtbonebe $TIMESTEP $ELE_SIZE $EL_X $EL_Y $EL_Z" >> jobfile
...
do
    ...
    do
	  ...
	  Exyz=$((EL_X*EL_Y*EL_Z))
 	  ...
 	  #then
	    qsub $1/job-bonebe'-es'$ELE_SIZE'ec'$Exyz'.job'
	  #fi
	done
done
