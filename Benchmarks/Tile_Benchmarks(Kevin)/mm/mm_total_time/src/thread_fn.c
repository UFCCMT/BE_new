...
void *thread_fn(void *arg)
{
  ...
  double bcast_s, bcast_e, bcast_d, scatter_s, scatter_e, scatter_d, compute_s, compute_e, compute_d, gather_s, gather_e, gather_d, barrier_s, barrier_e, barrier_d, barrier1_s, barrier1_e, barrier1_d;
  struct timespec st;
  ...
  { 
   ...
   // bcast_s=((double)st.tv_sec*1e6 + (double)st.tv_nsec*1e-3)-zero;    
    ...
    //Set start time
    //clock_gettime(CLOCK_THREAD_CPUTIME_ID,&st);
    //barrier_s=((double)st.tv_sec*1e6 + (double)st.tv_nsec*1e-3)-zero;  
	
	barrier(ID);
    
	//Set end time
    //clock_gettime(CLOCK_THREAD_CPUTIME_ID,&st);
    //barrier_e=((double)st.tv_sec*1e6 + (double)st.tv_nsec*1e-3)-zero;    
    //barrier_d=barrier_e-barrier_s;
    
    //if(ID==0)
    //{
    //  printf("Iteration %d barrier done\n",iterator);
     // printf("Time taken: %lf\n",barrier_d);
    //}
    ...
    //Set start time
    //clock_gettime(CLOCK_THREAD_CPUTIME_ID,&st);
    //barrier1_s=((double)st.tv_sec*1e6 + (double)st.tv_nsec*1e-3)-zero;  
	
	barrier(ID);
    
	//Set end time
    //clock_gettime(CLOCK_THREAD_CPUTIME_ID,&st);
    //barrier1_e=((double)st.tv_sec*1e6 + (double)st.tv_nsec*1e-3)-zero;    
    ...
    //---------------------------Log the data-----------------------------------
    //log_memory[iterator*nthreads*18+ID*15+BCAST_S]=bcast_s;
    //log_memory[iterator*nthreads*18+ID*15+BCAST_E]=bcast_e;
    //log_memory[iterator*nthreads*7+ID*7+BCAST_D]=bcast_d;
    
    //log_memory[iterator*nthreads*18+ID*15+BAR_S]=barrier_s;
    //log_memory[iterator*nthreads*18+ID*15+BAR_E]=barrier_e;
    //log_memory[iterator*nthreads*7+ID*7+BAR_D]=barrier_d;
	
	//log_memory[iterator*nthreads*18+ID*15+SCA_S]=scatter_s;
    //log_memory[iterator*nthreads*15+ID*15+SCA_E]=scatter_e;
    //log_memory[iterator*nthreads*7+ID*7+SCA_D]=scatter_d;
    
    //log_memory[iterator*nthreads*15+ID*15+COMP_S]=compute_s;
    //log_memory[iterator*nthreads*15+ID*15+COMP_E]=compute_e;
    //log_memory[iterator*nthreads*7+ID*7+COMP_D]=compute_d;
	
	//log_memory[iterator*nthreads*15+ID*15+BAR1_S]=barrier1_s;
    //log_memory[iterator*nthreads*15+ID*15+BAR1_E]=barrier1_e;
    //log_memory[iterator*nthreads*7+ID*7+BAR1_D]=barrier1_d;
    
    //log_memory[iterator*nthreads*15+ID*15+GATH_S]=gather_s;
    //log_memory[iterator*nthreads*15+ID*15+GATH_E]=gather_e;
    //log_memory[iterator*nthreads*7+ID*7+GATH_D]=gather_d;
	
	log_memory[iterator*nthreads*1+ID*1+TOTAL_D]=total_time_d;
