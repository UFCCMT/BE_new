...
void init(int argc, char *argv[])
{
  if(argc<5)
  {
    printf("Usage: ./mm_sparse <config_filename> <n> <E> <iterations> <logfile>\n");
    exit(EXIT_FAILURE);
  }

  // Initialize to negative values for error checking  
  nthreads=-1;
  n=-1;
  E=-1;
  xmax=-1;
...
//  printf("Creating memory for logs...\n");
  logfile=argv[5];
  log_memory=(double*)malloc(sizeof(double)*nthreads*5*iterations);
