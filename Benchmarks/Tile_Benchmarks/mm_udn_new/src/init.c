...
void init(int argc, char *argv[])
{
  if(argc<4)
  {
    printf("Usage: ./mm_sparse <config_filename> <datasize> <iterations> <logfile>\n");
    exit(EXIT_FAILURE);
  }
  ...
  logfile=argv[4];
  log_memory=(double*)malloc(sizeof(double)*nthreads*12*iterations);
