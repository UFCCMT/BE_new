...
void finalize()
{
  ...
  fprintf(fp,"Th %d GATHER_D\n", nthreads-1);



  for(count=0; count<iterations; count++)
  {
    for(count1=0; count1<nthreads*12-1; count1++)
      fprintf(fp,"%lf,",log_memory[count*nthreads*12+count1]);
    fprintf(fp,"%lf\n",log_memory[count*nthreads*12+nthreads*12-1]);
  }
  fclose(fp);
