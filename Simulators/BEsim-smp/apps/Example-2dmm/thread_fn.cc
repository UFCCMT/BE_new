...
void *thread_fn(void *arg){
  ...
  if(threadID==0)
    time=m.smp_gettime(threadID);
        ...
        //Broadcast across row 0, then down all cols
  ...
  if(threadID==0){
    for(int count=1; count<nprocs; count++){
      if(count<datasize%nprocs) //If datasize not evenly divisible by nprocs
  ...
  if(threadID==0)
    time=m.smp_gettime(threadID);
     ...
     //If datasize not evenly divisible by nprocs
