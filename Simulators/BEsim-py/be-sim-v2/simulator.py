...
class Simulator(object):
    ...
    def __init__( self, shell, programDirectory, lookupDirectory, interpolator,
                  probeGIDs, probeOrdinals, probeStates, showPrinting, outputfile, fpgaTrace ):
        ...
        self.fpgaTrace = fpgaTrace
        self.toOutput = None
        
        if outputfile:
            self.toOutput = open(outputfile, "wb")
    ...
    def reset(self):
        ...
        self.environment = Environment(setter, getter, self.fpgaTrace)
    ...
    def _lookup_(self, kind, operation, inputs):
        ...
        filename, templates, degree = self.operations[kind][operation]

        # Get the (maybe interpolated) outputs if there was a file to look up.
        outputs = (
            self.librarian.lookup(self.lookupDirectory + filename, inputs, degree)
    ...
    def _obtain_(self, executor, name):
        ...
        if name == "sim.incrementalTime":
            inc_time = self.environment.time - executor.lastRecordedTime
            executor.lastRecordedTime = self.environment.time
            return inc_time
    ...
    def _call_(self, executor, target, operation, *inputs):
        ...
        return Routine( targetGID, PID(PID_ROUTINE_PREFIX), self.shell,
                        ...
                        inputs, outputs, templates, executor.uniqueid() )
    ...
    def _comm_(self, executor, operation, size, ranks, tag):
        ...
        if operation == SEND:
            ...
            return [ Message( executor.gid, PID(PID_MESSAGE_PREFIX),
                              ...
                              source, ranks, size, tag, executor.uniqueid() ) ]
...
def new(args, shell):
    ...
    sim = Simulator( shell, args.programDirectory, args.lookupDirectory,
                     ...
                     args.probeStates, args.showPrinting, args.softwareOutput, args.fpgaTrace )
...
def sequential(args, count, suppressed):
    ...
    for i in range(count):
        ...
        if args.fpgaTrace:
            prev = []
            for gid in sim.environment.trace:
                for record in sim.environment.trace[gid]:
                    if record[1] == "comm":
                        for recv_record in sim.environment.trace[record[-1][-1][0]]:
        ...
        if args.fpgaTrace:
            with open(args.fpgaTrace, 'w') as fout:
                for gid in sim.environment.trace:
                    for record in sim.environment.trace[gid]:
                        fout.write(str(record))
                        fout.write("\n")
                        if record[1] == "comm":
                            record_n = (record[0], "call", record[2], "Send", [record[-1][0][1][0], record[-1][-1][1][-1]], record[-1][-1][-1])
...
if __name__ == "__main__":
    ...
    ap.add_argument('-FPGA', '--fpgaTrace', default=None,
                    help="print FPGA trace for a particular simulation")
                    
    ap.add_argument('-so', '--softwareOutput', default=None,
                    help="Print results of AppBEO")
    ...
    ap.add_argument('-i', '--interpolator', default="polynomial",
