...
class Parser(object):
    ...
    def _for_(self, node):
        ...
        self._impliedIterable_(forNode)
        self.gobble(REGION_RIGHT)
    ...
    def _impliedIterable_(self, node):
        """ EBNF :: iterable = "[" start-expression "," end-expression} "]" ;
                 ::           iterable-node
            AST  ::          /      |      \
                 ::  expression expression expression ...
        """
	...
	#So the first number should be added regardless
        node.append(iterableNode)

	#grab the first number -> start index
        startIndex = int(self.current.text)
        self.gobble(NUMBER)	#Hopefully, this will gobble/discard anything
	#discard the separator -> comma
        self.gobble(SEPARATOR)
	#grab the second number -> end index
        stopIndex = int(self.current.text) + 1 # I believe that the +1 is needed to make it actually exclusive still
#earlier, FOR(i[1,8]) led to a lt 7 command in the output machine code, which seems incorrect. 
        self.gobble(NUMBER)	#Hopefully, this will gobble/discard anything

        #Loop froms start index to end index
        for i in range(startIndex, stopIndex):
            #Create a number token to be added
            #tempToken = self.tokenize(str(i))
	    #Add that number token to the list
                #I think we may have to do something to whatever self.current is
	    #Do the line and colum number matter?
            loopToken = Token(0,0, NUMBER, str(i))
	    loopTokenIndex = self.tokenIndex
            self.tokens.insert( loopTokenIndex, loopToken )
            #self.tokens.insert( loopTokenIndex, Token(line, column, newKind, newString) )
            self._expression_(iterableNode)
	#Consume the right side of the operator (LIST_RIGHT)
