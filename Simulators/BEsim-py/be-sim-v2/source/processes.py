...
from framework import ( Process, Change, Timeout, Condition, Autoprocess,
                        Subprocess, Terminate, RecvWait, MsgArvl )

from containers import ( ChangeTemplate, TimeoutTemplate, ConditionTemplate,
                         Procrastinator, RecvTemplate, MsgArvlTemplate )
...
class Routine(Process):
    ...
    def __init__(self, gid, pid, shell, view, plot, time,
                 state, inputs, outputs, sequence, uid):
    ...
    def run(self):
        ...
        for template in self.sequence:
            ...
            if find(template.provision):
                ...
                elif isinstance(template, RecvTemplate):
                
                    event = RecvWait()
                    yield event

                elif isinstance(template, MsgArvlTemplate):
...
class Message(Process):
    ...
    def __init__( self, gid, pid, shell, view, time,
                  router, source, target, size, tag, uid ):
...
class Executor(Process):
    ...
    def run(self):
        ...
        while self.line < len(self.program):
            ...
            #if self.gid == 16 and instruction.kind == "call":
             #   import pdb
              #  pdb.set_trace()
    ...
    def _print_(self, string, *things):
        ...
        print_items = []
        for thing in things:
            if thing.startswith('"') or thing.startswith("'"): print_items.append(thing[1:-1].replace("\\n", "\n"))
            else: print_items.append(repr(self.registers[thing]))

        things = ''.join(print_items)
        ...
        if self.pview: print string + things
        if self.simulator.toOutput:
                 self.simulator.toOutput.write(things) # Not printing string which is always " " for convenient syntax
