...
do
	...
	do
		...
		echo '#SBATCH -o '$2'/matrix_mult.out'$CUR_MATRIX_SIZE'np'$CUR_NUM_TASKS'.out' >> jobfile
		echo '#SBATCH -e '$2'/matrix_mult.out'$CUR_MATRIX_SIZE'np'$CUR_NUM_TASKS'.err' >> jobfile
		echo " " >> jobfile
		echo "module load intel/2016.0.109 openmpi/1.10.2" >> jobfile
		echo " " >> jobfile
		echo "mpirun -np $CUR_NUM_TASKS ./mat_mult.o $CUR_SIZE"
...
do
	...
	do
		...
		sbatch $1/matrix_mult.out$CUR_MATRIX_SIZE'np'$CUR_NUM_TASKS'.job'
	done
done
